...
const updateCostumersEnquiry = async (req, res) => {
    try {
        ...
        if (status) {
            ...
            if (!statuses.includes(status)) return res.status(400).send({ status: false, msg: `status must be selected among ${statuses}` });
        }
        if (deliveryStatus) {
            let deliveryStatuses = ["processing", "shipped", "inTransit", "delivered"];
            if (!deliveryStatuses.includes(deliveryStatus)) return res.status(400).send({ status: false, msg: `deliveryStatus must be selected among ${deliveryStatuses}` });
        }
        ...
        let getcustomerEnquiryData = await CostumerEnquiryModel.findById(customerEnquiryID);
        if (!getcustomerEnquiryData) { return res.status(404).send({ satus: false, message: "no customer enquiry found" }); }
        let customerId = getcustomerEnquiryData.customerID?.toString();
        if (!customerId) { return res.status(404).send({ satus: false, message: "customerID not found" }); }
        let customerData = await CostumerModel.findById(customerId);
        if (!customerData) { return res.status(404).send({ satus: false, message: "customer not found" }); }
...
const trackEnquiry = async (req, res) => {
    ...
    try {
        ...
        if (!validator.isValidObjectId(customerEnquiryId)) {
            res.status(400).send({
                status: false, message: "Please provide valid id"
            })
        }
        let getcustomerEnquiryId = await CostumerEnquiryModel.findOne({ _id: customerEnquiryId });
        ...
        if (getcustomerEnquiryId.isDeleted == true) {
            return res.status(404).send({
                status: false, message: "id is already deleted not found"
            })
        }

        let getEnquiries = await CostumerEnquiryModel.find({ _id: customerEnquiryId })
...
module.exports = { EnquiryForm, getEnquiries, IndividualCostumerEnquiry, deleteCostumerEnquiry, countData, pendingData, rejectedData, approvedData, updateCostumersEnquiry, countOfInprocessingDelivery, countOfindeliveredDelivery, countOfinshippedDelivery, countOfinTransitDelivery, trackEnquiry }